...
jest.mock('@/components/atoms/Card/FeaturesCard', () => {
  return function MockFeatureCard({ title, description, icon, iconBgColor, iconTextColor }: any) {
    return (
      <div data-testid="feature-card" className={`${iconBgColor} ${iconTextColor}`}>
...
describe('KeyFeatures', () => {
  describe('Default Rendering', () => {
    ...
    it('renders the default subtitle', () => {
      expect(
        screen.getByText('Our platform offers a comprehensive suite of tools for certification management')
      ).toBeInTheDocument();
    });
    ...
    it('renders default feature descriptions', () => {
      expect(screen.getByText('Create tamper-proof certificates backed by blockchain technology')).toBeInTheDocument();
      expect(screen.getByText('Verify certificates instantly using ID or QR code scanning')).toBeInTheDocument();
      expect(screen.getByText('Manage all your certifications in one centralized dashboard')).toBeInTheDocument();
    });
  ...
  describe('Styling and Layout', () => {
    ...
    it('has proper header text styling', () => {
      ...
      expect(title).toHaveClass('text-3xl', 'md:text-4xl', 'font-bold', 'text-white', 'mb-4');
      expect(subtitle).toHaveClass('max-w-2xl', 'text-xl', 'leading-7', 'font-normal', 'text-[#9CA3AF]', 'mx-auto');
    });
  ...
  describe('Default Features Data', () => {
    ...
    it('has correct SVG structure for security icon', () => {
      const securityIcon = screen.getByText('Secure Certification')
    ...
    it('has correct SVG structure for verification icon', () => {
      const verificationIcon = screen.getByText('Instant Verification')
        .closest('[data-testid="feature-card"]')
        ?.querySelector('[data-testid="feature-icon"] svg');
    ...
    it('has correct SVG structure for management icon', () => {
      const managementIcon = screen.getByText('Comprehensive Management')
        .closest('[data-testid="feature-card"]')
        ?.querySelector('[data-testid="feature-icon"] svg');
  ...
  describe('Component Props Interface', () => {
    ...
    it('accepts optional subtitle prop', () => {
      const { rerender } = render(<KeyFeatures />);
      expect(screen.getByText('Our platform offers a comprehensive suite of tools for certification management')).toBeInTheDocument();
  ...
  describe('Feature Data Structure', () => {
    ...
    it('passes icon as ReactNode correctly', () => {
      const customIcon = <span data-testid="custom-react-node">Custom React Node</span>;
  ...
  describe('Default Features Content', () => {
    ...
    it('mentions verification capabilities', () => {
      expect(screen.getByText(/verify certificates instantly/i)).toBeInTheDocument();
    ...
    it('describes management features', () => {
      ...
      expect(screen.getByText(/manage all your certifications/i)).toBeInTheDocument();
    });
  });
  ...
  describe('SVG Icon Structure', () => {
    ...
    it('all SVG icons have consistent structure', () => {
      const svgIcons = document.querySelectorAll('[data-testid="feature-icon"] svg');
    ...
    it('SVG paths have proper stroke attributes', () => {
      const svgPaths = document.querySelectorAll('[data-testid="feature-icon"] svg path');
