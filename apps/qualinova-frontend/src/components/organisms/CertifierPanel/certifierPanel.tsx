...
import { Search, Plus, ArrowUpDown, FileText, CircleCheckBig, CircleX, Clock4, HelpCircle } from 'lucide-react';
...
type TabType = 'assigned' | 'templates' | 'help';
...
const CertifierPanel = () => {
    ...
    const renderTabButton = (tabKey: TabType, label: string, icon?: React.ReactNode) => (
        <button
                ...
                <div className="mb-6">
                    <div className="bg-gray-800 rounded-lg p-1 w-fit">
                        <nav className="flex flex-wrap gap-1">
                            {renderTabButton('assigned', 'Assigned Certificates')}
                            {renderTabButton('templates', 'Certificate Templates')}
                            {renderTabButton('help', 'Help', <HelpCircle className="h-4 w-4" />)}
                        </nav>
                    </div>
                </div>
